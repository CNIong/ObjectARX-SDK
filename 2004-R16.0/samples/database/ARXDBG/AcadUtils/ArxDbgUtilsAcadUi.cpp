    ...
    if (stat == ArxDbgUiPrBase::kNone) {
        ...
        if (prHandle.go() == ArxDbgUiPrBase::kOk) {
            if (ArxDbgUtils::handleStrToObjId(acdbHostApplicationServices()->workingDatabase(),
                ...
                ArxDbgUtils::alertBox("Couldn't find that handle!");
                return false;
            }
            return true;
        }
...
ArxDbgUtils::dwgVersionToStr(AcDb::AcDbDwgVersion type, CString& str)
{
    ...
    if (type == AcDb::kDHL_MC0_0)
        ...
        verStr = _T("kDHL_1500  (R15 Development)");
    else if (type == AcDb::kDHL_1015)
        verStr = _T("kDHL_1015  (R15 Final Version)");
    else if (type == AcDb::kDHL_1800)
        verStr = _T("kDHL_1800  (Red Deer Development Version)");
    /* else if (type == AcDb::kDHL_1018)
        verStr = _T("kDHL_1018 (R18  Final Version)"); */
    else {
        ASSERT(0);
        ASSERT(AcDb::kDHL_Newest == AcDb::kDHL_1800); // update needed above
        verStr = _T("*Unknown*");
    }
...
ArxDbgUtils::saveFormatToStr(AcApDocument::SaveFormat type, CString& str)
{
    ...
    else if (type == AcApDocument::kR15_dwg)
        str = _T("AutoCAD R15 Drawing (*.dwg)");
    else if (type == AcApDocument::kR15_dxf)
        str = _T("AutoCAD R15 DXF (*.dxf)");
    else if (type == AcApDocument::kR15_Template)
        str = _T("R15 Drawing Template File (*.dwt)");
    else if (type == AcApDocument::kR18_dwg)
        str = _T("AutoCAD R18 Drawing (*.dwg)");
    else if (type == AcApDocument::kR18_dxf)
        str = _T("AutoCAD R18 DXF (*.dxf)");
    else if (type == AcApDocument::kR18_Template)
        str = _T("R18 Drawing Template File (*.dwt)");
    else {
        ASSERT(0);
        str = _T("*Unknown*");
    }
...
{
    ArxDbgUiPrEntity prEnt("Select an entity (or RETURN for Handle)", NULL);
    prEnt.setAllowNone(true);

    ArxDbgUiPrBase::Status stat = prEnt.go();
    if (stat == ArxDbgUiPrBase::kNone) {
        ArxDbgUiPrString prHandle("Handle of database object", false);
        if (prHandle.go() == ArxDbgUiPrBase::kOk) {
