                               ...
                               AcDbVoidPtrArray& curveSegments) const
{
    ...
    for (int i = 0; i < parameters.length(); i++) {
        ...
        AOK(getPointAtParam(nextParam, pt));
        pts.append(pt);
...
AsdkPoly::list() const
{
    ...
    acutPrintf("%18s%16s %d \n", /*MSG0*/"", "Number Of Sides:", mNumSides);

    acutPrintf("%18s%16s %-9.16q0 \n", 
               /*MSG0*/"", "Circumference:", sideLength * mNumSides);

    double areaVal;
    getArea(areaVal);
    acutPrintf("%18s%16s %-9.16q0 \n", /*MSG0*/"", "Area:", areaVal);
    
    acutPrintf("%18s%16s ", /*MSG0*/"", "Center:");
...
Acad::ErrorStatus   AsdkPoly::getClassID(CLSID* pClsid) const
{
    assertReadEnabled();
...
void updateRegistry()
{
	...
	// Get the physical location of the DBX...
	...
	appInfo.setAppDesc("AsdkPolyObj version 2.0"); // Description
	appInfo.setLoadReason(// Specify when we want this to load.
		AcadApp::LoadReasons(
		AcadApp::kOnProxyDetection |
		AcadApp::kOnLoadRequest));
	
	appInfo.writeToRegistry(true,true);// Write appInfo to the registry.  Include DBX section
}


void changeAppNameCallback(const AcRxClass* classObj, char*& newAppName,
    int saveVer)
{
    if (saveVer == AcDb::kDHL_1014 && classObj == AsdkPoly::desc())
        acutNewString("AsdkPolyOBJ"
                      "|Product Desc:     PolyCAD ARX App For Polygon Entity"
                      "|Company:          Autodesk,Inc."
                      "|WEB Address:      www.autodesk.com", newAppName);
}
...
acrxEntryPoint(AcRx::AppMsgCode msg, void* pkt)
{
    ...
    case AcRx::kUnloadAppMsg:
        ...
        AcRxObject *obj = acrxServiceDictionary->remove("AsdkPoly");
        if (obj != NULL)
            delete obj;
