...
{
	using std::string;
	return static_cast<string*>(pThis->appData())->c_str();
						...
						int					 contextFlags)
{
	...
	string *psApppData = static_cast<string *>(pthis->appData());
					 ...
					 ContextMenuItemIndexPtr	&cb)
{
	...
	if(menu){
		::AppendMenu(menu,MF_STRING,1,"Print Center Coordinate for the entity");
		::AppendMenu(menu,MF_STRING,2,"Normal Coordinate for the entity");
								...
								int							 gripSize)
{
	double radius(0.2);
	AcGeVector3d normal(0,0,1);
	std::string sAppData(static_cast<char *>(pThis->appData()));
