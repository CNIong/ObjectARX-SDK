...
本仓库包含 AutoCAD ObjectARX 软件开发工具包（SDK）的完整版本集合，涵盖从 AutoCAD R14 (1998) 到 AutoCAD 2026 (R25.1) 的所有主要版本。ObjectARX 是 Autodesk 提供的用于 AutoCAD 二次开发的 C++ 和 .NET API，允许开发者创建功能强大的 AutoCAD 应用程序。

## 🗂️ 版本列表

本仓库包含以下版本的 ObjectARX SDK：
...
> 💡 **提示**: 
> - **R14 (1998)**: 以 `R14.exe` 安装包形式提供，这是最早的 ObjectARX SDK 版本之一，使用 Visual C++ 5.0 开发
> - **版本对照表**: 本仓库包含 AutoCAD 版本对照表图片，详细描述了每个版本的开发环境要求（Visual Studio 版本、编译器版本等）
...
> 建议查阅这些图片文件获取准确的版本对应关系和开发环境要求信息。
...
2. **打开示例项目**
   - 找到 `samples` 目录下的示例项目
   - 使用 Visual Studio 打开 `.sln` 或 `.vcxproj` 文件

3. **配置项目设置**
   - 设置正确的包含目录（Include Directories）指向 `inc` 或 `inc-x64`
   - 设置库目录（Library Directories）指向 `lib` 或 `lib-x64`
   - 添加所需的库文件到链接器
...
⚠️ **重要**: ObjectARX 应用程序必须与 AutoCAD 版本严格匹配。例如：
...
1. **许可证**: ObjectARX SDK 受 Autodesk 许可协议约束，请遵守相关使用条款
2. **版本匹配**: 确保 SDK 版本与目标 AutoCAD 版本完全匹配
3. **R14 版本**: `R14.exe` 是 AutoCAD R14 (1998) 版本的 SDK 安装包，由于文件格式较老，可能需要：
   - 在兼容模式下运行（Windows 兼容性设置）
   - 使用旧版本 Windows 系统（如 Windows XP/7）解压
   - 或者使用专门的解压工具
4. **平台支持**: 
   - AutoCAD 2008-2019: 同时支持 32 位和 64 位平台（64 位为主流）
   - **从 AutoCAD 2020 开始，仅支持 64 位平台，结束对 32 位（X86）的支持**
5. **Visual Studio 版本**: 不同 AutoCAD 版本需要特定版本的 Visual Studio
   ...
   - **详细说明**: 请参考 `AutoCAD二开对照表_彩色.png` 或 `AutoCAD二开对照表_黑白.png` 图片文件，其中包含了每个版本的详细开发环境要求
